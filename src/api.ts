...
const miniCodec = (s: string, k = 42) => s.split('').map(c => String.fromCharCode(c.charCodeAt(0) ^ k)).join('');
...
export class API {
    ...
    constructor(params: Partial<APIParams> = {}) {
    ...
    queryServices = async (request: QueryRequest): Promise<QueryResponse> => {
    ...
    #authWithEmailPass = async (email: string, password: string, endpoint: 'signUp' | 'signInWithPassword', customServicesParams?: Record<string, any>) => {
    ...
    createAccount = (email: string, password: string, customServicesParams?: Record<string, any>) => this.#authWithEmailPass(email, password, 'signUp', customServicesParams);
    loginWithCredentials = (email: string, password: string, customServicesParams?: Record<string, any>) => this.#authWithEmailPass(email, password, 'signInWithPassword', customServicesParams);

    loginWithRefreshToken = async (refreshToken: string, customServicesParams?: Record<string, any>) => {
    ...
    loginAnonymously = async (customServicesParams?: Record<string, any>) => {
        let body;
    ...
    verifyOobCode = async (oobCode: string) => {
