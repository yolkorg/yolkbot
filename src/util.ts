...
import { CreatedGun, Gun } from './constants/guns';
import type { MapJSON } from './constants/maps';

export const createGun = (baseGun: Gun): CreatedGun => {
    const createdGun = {
        ...baseGun,
        ammo: {
            ...baseGun.ammo,
            rounds: baseGun.ammo.capacity,
            storeMax: baseGun.ammo.store
        }
    } as CreatedGun;
...
export const createError = <T>(errorEnum: T): { ok: false, error: T } => ({ ok: false, error: errorEnum });

const attemptFetch = async (url: string, options = {}, retries = 3, backoff = 300) => {
    try {
...
export const fetchMap = async (name: string, hash: string): Promise<MapJSON> => {
...
export const initKotcZones = (meshData: Position3D[]) => {
