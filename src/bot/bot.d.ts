import { Character, GamePlayer, View } from './GamePlayer';
...
export class Bot {
    ...
    on(event: 'grenadeExplode', cb: (item: Item | number, pos: Position3D, damage: number, radius: number) => void): void;
    ...
    on(event: 'playerMove', cb: (player: GamePlayer, oldPosition: Position3D, newPosition: Position3D) => void): void;
    ...
    on(event: 'playerThrowGrenade', cb: (player: GamePlayer, position: Position3D, dPosition: Position3D) => void): void;
    on(event: 'respawnDenied', cb: () => void): void;
    on(event: 'rocketHit', cb: (pos: Position3D, damage: number, blastRadius: number) => void): void;
    ...
    on(event: 'spawnItem', cb: (itemType: number, itemPosition: Position3D, itemId: number) => void): void;
