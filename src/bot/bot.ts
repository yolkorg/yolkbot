...
import { Challenge, Challenges } from '../constants/challenges.js';
import { findItemById } from '../constants/findItemById.js';
import { MapJSON, Maps } from '../constants/maps.js';
import { Region, Regions } from '../constants/regions.js';
...
import processSyncThemPacket, { FramesBetweenSyncs, StateBufferSize } from '../packets/syncThem.js';
...
import { Position3D } from '../common.js';
...
export class Bot {
    ...
    afkKickInterval: NodeJS.Timeout | null = null;
    renewSessionInterval: NodeJS.Timeout | null = null;
    ...
    dispatch(dispatch, _isEmit: boolean) {
        ...
        this.errorLogger(`${_isEmit ? 'emit' : 'dispatch'}: validation failed for dispatch ${dispatch.constructor.name}`);
    ...
    async findPublicGame(region, mode) {
        ...
        if (typeof mode === 'number') {
            if (mode in GameMode) computedModeId = mode;
            else return createError(GameFindError.InvalidMode);
        } else if (typeof mode === 'string') {
            const modeEntry = Object.keys(GameMode)
                .filter(key => isNaN(Number(key)))
                .find(key => key.toLowerCase() === mode.toLowerCase());
    ...
    update() {
        ...
        for (let i = 0; i < players.length; i++) {
            ...
            if (player.playing && player.hp > 0) {
                const hasOverHeal = player.shellStreaks.includes(ShellStreak.OverHeal);
